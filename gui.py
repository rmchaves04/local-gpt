...
@app.route('/')
def index():
    return render_template('index.html', available_models = MODELS, message_history=message_history)
...
def submit():
    ...
    if history:
        messages.append({
            "role": "system",
            "content": build_history_prompt(prompt)
        })
    else:
        messages.append({
            "role": "user",
    ...
    return render_template('submit.html', response=clean_res, cost=cost, message_history=message_history)
